...
/* Initialize with gap penalties, skipped in SW-algo */
...
function fillMatrices(X: string, Y: string, F: number[][], trace: number[][], m: number, n: number) {
    for (let i = 1; i <= m; i++) {
        for (let j = 1; j <= n; j++) {
            fillCellSW(X, Y, F, trace, i, j);
        }
    }
    return { F, trace };
}

/* Fill cell using SW-algo, main difference is no negative values */
function fillCellSW(X: string, Y: string, F: number[][], trace: number[][], i: number, j: number) {
    let score: number, tmp: number;
    if (X[i - 1] === Y[j - 1]) {
        score = Math.max(0, F[i - 1][j - 1] + MATCH_SCORE);
    } else {
        score = Math.max(0, F[i - 1][j - 1] + MISMATCH_SCORE);
    }
    trace[i][j] = DIAG;

    tmp = Math.max(0, F[i - 1][j] + GAP_PENALTY);
    if (tmp > score) {
        score = tmp;
        trace[i][j] = UP;
    }

    tmp = Math.max(0, F[i][j - 1] + GAP_PENALTY);
    if (tmp > score) {
        score = tmp;
        trace[i][j] = LEFT; 
    }
...
/* Fill cell depending on adjacent cells, NW-algo */
...
/* Main function */
function localAlignment(x: string, y: string) {
    const X = x;
    const Y = y;
    ...
    const {rowIndex, columnIndex} = findMaxValueIndex(filledF);
    console.log('Row: '+rowIndex, 'Col: '+columnIndex, 'Score: '+filledF[rowIndex][columnIndex])

    //tracebackAlignment(X, Y, filledTrace, m, n);
    tracebackAlignment(X, Y, filledTrace, rowIndex, columnIndex); /* Traceback from largest value */
}

/* Variable sequences */
const sequence1 = 'PAWHEAE';
const sequence2 = 'HDAGAWGHEQ';
localAlignment(sequence1, sequence2);
...
function findMaxValueIndex(matrix: number[][]) {
    let max = matrix[0][0];
    let rowIndex = 0;
    let columnIndex = 0;
